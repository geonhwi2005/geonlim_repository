...
def preprocess(img, target_h=256, max_w=1024):
    """
    그레이스케일 이미지 리사이즈 → 패딩 캔버스 삽입 → 90도 회전
    ...
    canvas[:, :new_w] = img
    # CRNN 입력 차원(H,W)을 (W,H)로 바꾸기 위해 회전
...
class DataGenerator(Sequence):
    def __init__(self, df, img_dir, batch_size, target_h=256, max_w=1024):
        self.df = df
